...
#ifdef IS_TEST_NET
#define STEEMIT_INIT_PRIVATE_KEY                (fc::ecc::private_key::regenerate(fc::sha256::hash(std::string("my_init_key")))) //(SV)
#define STEEMIT_INIT_PUBLIC_KEY_STR             (std::string( steemit::protocol::public_key_type(STEEMIT_INIT_PRIVATE_KEY.get_public_key()) ))
#define STEEMIT_CHAIN_ID                        (fc::sha256::hash("my_testnet")) // (SV)

//#define VESTS_SYMBOL  (uint64_t(6) | (uint64_t('V') << 8) | (uint64_t('E') << 16) | (uint64_t('S') << 24) | (uint64_t('T') << 32) | (uint64_t('S') << 40)) ///< VESTS with 6 digits of precision
#define VESTS_SYMBOL  (uint64_t(6) | (uint64_t('C') << 8) | (uint64_t('A') << 16) | (uint64_t('C') << 24) | (uint64_t('A') << 32) | (uint64_t('O') << 40)) ///< VESTS with 6 digits of precision  // (SV)
...
#define STEEMIT_ADDRESS_PREFIX                  "XYZ" // (SV)

#define STEEMIT_GENESIS_TIME                    (fc::time_point_sec(1234567890)) // (SV)
#define STEEMIT_MINING_TIME                     (fc::time_point_sec(1234567890)) // (SV)
...
#define STEEMIT_ORIGINAL_MIN_ACCOUNT_CREATION_FEE 1
...
#define STEEMIT_ORIGINAL_MIN_ACCOUNT_CREATION_FEE  1 // modified by SV
#define STEEMIT_MIN_ACCOUNT_CREATION_FEE           1 // modified by SV
...
#define STEEMIT_INIT_MINER_NAME                 "alice" // modified by SV <-- "initminer"
...
#define STEEMIT_HARDFORK_REQUIRED_WITNESSES     1 // 17 of the 21 dpos witnesses (20 elected and 1 virtual time) required for hardfork. This guarantees 75% participation on all subsequent rounds. // (SV)
...
#define STEEMIT_INIT_SUPPLY                     int64_t(1234567890) // (SV)
